...
import {
  ...
  adjustRiderWalletInFirebase,
  createPayoutRequestInFirebase,
...
import { Driver, Rider, Trip, SurgeZone, PricingConfig, SupportTicket, LocationPoint, LiveDriverLocation, SafetyReport, LiveActivityFeedItem, RideRating, AdminNotification, PayoutRequest, OperationalRecommendation, WalletTransaction } from './types';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubWalletTransactions = subscribeToWalletTransactions((txs) => {
  ...
  const handleAdjustDriverWallet = async (driverId: string, amount: number, meta?: Partial<WalletTransaction>) => {
    let targetNewBalance = 0;
    const driver = drivers.find((d) => d.id === driverId);
    const prevBalance = driver?.walletBalance || 0;
    targetNewBalance = prevBalance + amount;
    ...
    await adjustDriverWalletInFirebase(driverId, targetNewBalance, {
      userName: driver?.fullName || 'Driver',
  ...
  // Adjust Passenger / Rider Wallet balance
  const handleAdjustRiderWallet = async (riderId: string, amount: number, meta?: Partial<WalletTransaction>) => {
    let targetNewBalance = 0;
    const rider = riders.find((r) => r.id === riderId);
    const prevBalance = rider?.walletBalance || 0;
    targetNewBalance = prevBalance + amount;

    setRiders((prev) =>
      prev.map((r) => {
        if (r.id === riderId) {
          return { ...r, walletBalance: targetNewBalance };
        }
        return r;
      })
    );
    await adjustRiderWalletInFirebase(riderId, targetNewBalance, {
      userName: rider?.fullName || 'Passenger',
      userPhone: rider?.phone || '',
      amount: Math.abs(amount),
      previousBalance: prevBalance,
      ...meta,
    });
    showToast(`Passenger wallet updated: Rs. ${targetNewBalance.toLocaleString('en-PK', { minimumFractionDigits: 2 })} in Firebase.`);
  };

  // Create new Driver Payout Request
  const handleCreatePayoutRequest = async (payoutData: Partial<PayoutRequest>) => {
    const payoutId = await createPayoutRequestInFirebase(payoutData);
    showToast(`Payout request #${payoutId.slice(0, 8)} created successfully in Firebase.`);
  };
        ...
        <main className="flex-1 flex flex-col min-h-0 overflow-hidden">
          ...
          {activeTab === 'users' && (
            <UserManagement
              ...
              onAdjustRiderWallet={handleAdjustRiderWallet}
