...
interface UserDetailModalProps {
  ...
  onOpenWalletAdjustment?: (user: Driver | Rider, userType: 'driver' | 'rider') => void;
}
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fadeIn">
      <div className="bg-white rounded-3xl max-w-xl w-full p-6 shadow-2xl space-y-5 border border-slate-200 max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center space-x-3.5">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-[11px] text-slate-500 mt-1">
                ...
                <span>
                  {isDriver
                    ? `${driver?.totalTrips ?? 0} completed ride${(driver?.totalTrips ?? 0) === 1 ? '' : 's'}`
                    : `${rider?.totalRides ?? 0} ride${(rider?.totalRides ?? 0) === 1 ? '' : 's'} taken`}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="bg-emerald-50/60 p-4 rounded-2xl border border-emerald-200/80 flex items-center justify-between">
              ...
              {onOpenWalletAdjustment && (
                <button
                  onClick={() => {
                    onClose();
                    onOpenWalletAdjustment(driver, 'driver');
                  }}
              ...
              <div className="bg-slate-900 text-white p-4 rounded-2xl border border-slate-800 space-y-2 text-xs">
                <div className="flex items-center justify-between text-slate-300">
                  ...
                  <span className="text-[10px] bg-slate-800 text-emerald-400 px-2 py-0.5 rounded font-mono">
                    {driver.telemetry.androidVersion || 'Android 13'}
                  </span>
                </div>
            ...
            <div className="bg-emerald-50/60 p-4 rounded-2xl border border-emerald-200/80 space-y-2">
              <div className="flex items-center justify-between">
                <h4 className="font-bold text-emerald-950 flex items-center gap-1.5">
                  ...
                  Payment Preferences & Wallet
                </h4>
                {onOpenWalletAdjustment && (
                  <button
                    onClick={() => {
                      onClose();
                      onOpenWalletAdjustment(rider, 'rider');
                    }}
                    className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg text-[11px] shadow-2xs transition-all flex items-center gap-1 cursor-pointer"
                  >
              ...
              <div className="grid grid-cols-2 gap-2 text-emerald-900 text-[11px]">
                ...
                <div>
                  ...
                  <span className="font-bold font-mono text-emerald-800 text-sm">
                    Rs. {(rider.walletBalance ?? 0).toLocaleString('en-PK', { minimumFractionDigits: 2 })}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2.5">
          ...
          <button
            onClick={onClose}
            className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl transition-all ml-auto cursor-pointer"
          >
            Close Dossier
          </button>
        </div>
