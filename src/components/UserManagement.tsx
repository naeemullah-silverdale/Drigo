...
import { Driver, Rider, VehicleType, Trip } from '../types';
import {
  Users,
  ShieldCheck,
  Smartphone,
...
interface UserManagementProps {
  ...
  onAdjustDriverWallet: (driverId: string, amount: number, meta?: any) => void;
  onAdjustRiderWallet?: (riderId: string, amount: number, meta?: any) => void;
...
export const UserManagement: React.FC<UserManagementProps> = ({
  ...
  // Modals State using IDs & objects to guarantee live reactive sync
  ...
  const [selectedWalletUser, setSelectedWalletUser] = useState<{ user: Driver | Rider; type: 'driver' | 'rider' } | null>(null);
  ...
  // Enrich Drivers with live completed trips & Firestore Verification State
  const enrichedDrivers = useMemo(() => {
    return drivers.map((driver) => {
      // Calculate live completed rides from trips if present
      const driverTripsCount = (trips || []).filter((t) => {
        if (!t || typeof t !== 'object') return false;
        const driverMatch = t.driverId === driver.id || (t as any).driver_id === driver.id || t.driver?.id === driver.id;
        const statusLower = String(t.status || '').toLowerCase();
        const isCompleted = statusLower === 'completed' || statusLower === 'finished' || statusLower === 'ended' || (t as any).isCompleted === true;
        return driverMatch && isCompleted;
      }).length;
      const totalTrips = Math.max(driver.totalTrips || 0, driverTripsCount);
  ...
  }, [drivers, verificationsData, trips]);

  // Enrich Riders with live completed rides
  const enrichedRiders = useMemo(() => {
    return riders.map((rider) => {
      const riderTripsCount = (trips || []).filter((t) => {
        if (!t || typeof t !== 'object') return false;
        const riderMatch = t.riderId === rider.id || (t as any).rider_id === rider.id || (t as any).passengerId === rider.id || t.rider?.id === rider.id;
        const statusLower = String(t.status || '').toLowerCase();
        const isCompleted = statusLower === 'completed' || statusLower === 'finished' || statusLower === 'ended' || (t as any).isCompleted === true;
        return riderMatch && isCompleted;
      }).length;
      const totalRides = Math.max(rider.totalRides || 0, riderTripsCount);
      return {
        ...rider,
        totalRides,
      };
  ...
  const activeUserDetail = useMemo(() => {
    ...
    if (selectedUserDetailInfo.type === 'driver') {
      ...
      const r = enrichedRiders.find((rd) => rd.id === selectedUserDetailInfo.id);
      return r ? { user: r, type: 'rider' as const } : null;
    }
  }, [selectedUserDetailInfo, enrichedDrivers, enrichedRiders]);

  const activeWalletTarget = useMemo(() => {
    if (!selectedWalletUser) return null;
    if (selectedWalletUser.type === 'driver') {
      const d = enrichedDrivers.find((dr) => dr.id === selectedWalletUser.user.id) || selectedWalletUser.user;
      return { user: d, type: 'driver' as const };
    } else {
      const r = enrichedRiders.find((rd) => rd.id === selectedWalletUser.user.id) || selectedWalletUser.user;
      return { user: r, type: 'rider' as const };
    }
  }, [selectedWalletUser, enrichedDrivers, enrichedRiders]);
  ...
  const unifiedUsers = useMemo(() => {
    ...
    enrichedDrivers.forEach((d) => {
      ...
      list.push({
        ...
        joinedDate: d.joinedDate || '2024-03-15',
        rating: d.rating,
    ...
    enrichedRiders.forEach((r) => {
      list.push({
        ...
        joinedDate: r.joinedDate || '2024-05-10',
        rating: r.rating,
    ...
    return list;
  }, [enrichedDrivers, enrichedRiders]);
    ...
    <div className="space-y-4">
                          ...
                          >
                            ...
                            <td className="py-3 px-4 text-right">
                              <div className="flex items-center justify-end space-x-1.5">
                                ...
                                {isDriver && driverObj && (
                                  <button
                                    onClick={() => setSelectedWalletUser({ user: driverObj, type: 'driver' })}
                                ...
                                {/* Adjust Wallet / Top-Up Button for Passengers */}
                                {!isDriver && (
                                  <button
                                    onClick={() => setSelectedWalletUser({ user: item.rawObject as Rider, type: 'rider' })}
                                    className="p-1.5 text-slate-600 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors cursor-pointer"
                                    title="Top-Up / Refund Passenger Wallet"
                                  >
                                    <DollarSign className="w-4 h-4" />
                                  </button>
                                )}
      ...
      {activeUserDetail && (
        <UserDetailModal
          ...
          onOpenWalletAdjustment={(u, uType) => setSelectedWalletUser({ user: u, type: uType })}
        />
      )}
      ...
      {/* MODAL: Adjust Driver or Passenger Wallet Balance */}
      {activeWalletTarget && (
        <AdjustWalletModal
          user={activeWalletTarget.user}
          userType={activeWalletTarget.type}
          onClose={() => setSelectedWalletUser(null)}
          onConfirmAdjustment={(userId, amount, txMeta) => {
            if (activeWalletTarget.type === 'driver') {
              onAdjustDriverWallet(userId, amount, txMeta);
            } else if (onAdjustRiderWallet) {
