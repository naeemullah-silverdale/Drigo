...
export function normalizeFirebaseUser(key: string, val: any): { driver?: Driver; rider?: Rider; rawRole: string } {
  ...
  const totalTrips = typeof val.totalTrips === 'number' ? val.totalTrips : typeof val.totalRides === 'number' ? val.totalRides : 0;
  const walletBalance = typeof val.walletBalance === 'number' ? val.walletBalance : typeof val.balance === 'number' ? val.balance : 0;
...
/**
 * Real-time listener for ALL users from /users node (with fallbacks to /drivers & /riders)
 */
    ...
    const unsubUsers = onValue(usersRef, (snapshot) => {
      ...
      if (rawUsers && typeof rawUsers === 'object') {
        Object.entries(rawUsers).forEach(([key, val]) => {
          if (val && typeof val === 'object') {
            const normalized = normalizeFirebaseUser(key, val);
            if (normalized.driver) driversMap[key] = normalized.driver;
            if (normalized.rider) ridersMap[key] = normalized.rider;
          }
        });
      }

      // Check legacy /drivers node
      const driversRef = ref(db, 'drivers');
      onValue(driversRef, (dSnap) => {
        const dVal = dSnap.val();
        if (dVal && typeof dVal === 'object') {
          Object.entries(dVal).forEach(([key, val]) => {
            if (val && typeof val === 'object' && !driversMap[key]) {
              const normalized = normalizeFirebaseUser(key, val);
              if (normalized.driver) driversMap[key] = normalized.driver;
            }
          });
        ...
        // Check legacy /riders node
        const ridersRef = ref(db, 'riders');
        onValue(ridersRef, (rSnap) => {
          const rVal = rSnap.val();
          if (rVal && typeof rVal === 'object') {
            Object.entries(rVal).forEach(([key, val]) => {
              if (val && typeof val === 'object' && !ridersMap[key]) {
                const normalized = normalizeFirebaseUser(key, val);
                if (normalized.rider) ridersMap[key] = normalized.rider;
              }
            });
          ...
          // Check /wallets node for wallet balance overlays
          const walletsRef = ref(db, 'wallets');
          onValue(walletsRef, (wSnap) => {
            const wVal = wSnap.val();
            if (wVal && typeof wVal === 'object') {
              Object.entries(wVal).forEach(([key, val]) => {
                const balance = typeof val === 'number' ? val : (val as any)?.balance || (val as any)?.amount;
                if (typeof balance === 'number') {
                  if (driversMap[key]) driversMap[key].walletBalance = balance;
                  if (ridersMap[key]) ridersMap[key].walletBalance = balance;
                }
              });
            }
      ...
      console.warn('RTDB users listener note:', err);
    });
  ...
  if (firestore) {
    ...
    const unsub = onSnapshot(colRef, (snapshot) => {
      ...
      snapshot.forEach(docSnap => {
        const normalized = normalizeFirebaseUser(docSnap.id, docSnap.data());
        if (normalized.driver) driversList.push(normalized.driver);
        if (normalized.rider) ridersList.push(normalized.rider);
      });
      callback({ drivers: driversList, riders: ridersList });
    }, (err) => {
...
/**
 * Adjust Driver Wallet in Firebase
 */
export async function adjustDriverWalletInFirebase(
  ...
  if (db) {
    try {
      await update(ref(db, `drivers/${driverId}`), { walletBalance: newBalance });
      success = true;
    } catch (err) {
      console.warn('RTDB adjust wallet error:', err);
    }
  }

  if (firestore) {
    try {
      await updateDoc(doc(firestore, 'drivers', driverId), { walletBalance: newBalance });
      success = true;
    } catch (err) {
      console.warn('Firestore adjust wallet error:', err);
    }
  }
