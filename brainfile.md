...
- Singleton handling network and database interactions:
  ...
  - City to City departures stream (`observeAllPlannedDepartures`, `submitCityToCityOffer`)
...
- **Recent Commit History:**
  - `feat: enhance City to City passenger departures UI with responsive cards, top category badges, departure route timeline container, seat dot indicators, and full theme integration`
...
### 📋 Responsive UI Fix Applied & Verified (COMPLETED)
1. **Top Turn-by-Turn Banner**:
   - Set `maxLines = 2` with `TextOverflow.Ellipsis` for `currentNavInstruction` with clean spacing, ensuring live turn instructions never truncate to `Drive to ...`.
2. **2-Row Responsive Passenger Header**:
   - **Row 1**: Avatar + Passenger Name (`weight(1f)`) + Star Rating Badge (`★ 4.9`) + Green Fare Pill (`PKR $totalCashToCollect`).
   - **Row 2**: Equal weight full-width action bar containing 4 touch-friendly action buttons (`Navigate`, `Call`, `Chat`, `Share`) at `height = 32.dp`.
   - Eliminates text squishing and ensures passenger name and fare are 100% legible on 320dp–360dp budget devices.
3. **Zero-Overlap Recenter FAB**:
   - Anchored `driver_recenter_location_btn` at `Alignment.TopEnd` (`top = 165.dp`), moving it out of the bottom sheet's sweep zone and preventing overlap with bottom sheet headers or badges.
4. **Build Verification**:
   - Compiled cleanly with `compile_applet` (0 build errors).

---

## 18. DIAGNOSTIC & SPECIFICATION: USER MODE PERSISTENCE (PASSENGER VS DRIVER MODE) ACROSS APP RESTARTS

### 🛑 Problem Diagnosis & Issue Verification (CONFIRMED)
- **Issue Confirmed**: **YES, ISSUE EXISTS.**
- **Root Cause Identified**:
  1. **No SharedPreferences Persistence for User Mode**: `_userMode` in `MainViewModel` (line 68) is initialized hardcoded as `MutableStateFlow(UserMode.PASSENGER)`. Unlike `_isDriverOnline` (which calls `getPersistedDriverOnlineState()`), `_userMode` does not read or restore from local `SharedPreferences`.
  2. **`setUserMode(mode)` Lacks Local Storage Write**: When the user switches mode via `setUserMode(mode)` (lines 294–305), `MainViewModel` updates `_userMode.value` and attempts an async network write to Firebase `users/$uid/mode`, but **never saves the selected mode to `SharedPreferences`**.
  3. **App Restart Fallback Behavior**: When the user closes and re-opens the app:
     - `MainViewModel` is re-created with default `_userMode.value = PASSENGER`.
     - `fetchUserModeFromDb(uid)` runs asynchronously over the network. If the device is offline, on a slow connection, or if RTDB returns error/null (or during database URL mismatch between default instance and explicit RTDB URL `https://drigo-8b15c-default-rtdb.firebaseio.com`), `fetchUserModeFromDb` falls back to `_userMode.value = PASSENGER`.
     - Result: Any logged-in user who switched to `DRIVER` mode loses their mode selection upon app close/re-open and is forced back into `PASSENGER` mode.

### 📋 Implementation Completed & Verified
1. **SharedPreferences Helper Functions Added to `MainViewModel`**:
   - `getPersistedUserMode(): UserMode`: Retrieves `"user_mode"` from `drigo_driver_prefs` (`SharedPreferences`). Defaults to `UserMode.PASSENGER`.
   - `persistUserMode(mode: UserMode)`: Writes `"user_mode"` (`"DRIVER"` or `"PASSENGER"`) synchronously to `drigo_driver_prefs`.
2. **Instant Local State Restoration on App Startup**:
   - Initialized `_userMode = MutableStateFlow(getPersistedUserMode())`.
   - On app startup, `MainViewModel` immediately boots into the user's previously chosen mode (`DRIVER` or `PASSENGER`) without waiting for network calls or resetting to `PASSENGER`.
3. **Synchronized Mode Switching & Resilient Remote Sync**:
   - Updated `setUserMode(mode: UserMode)` to call `persistUserMode(mode)` immediately whenever the user toggles modes.
   - Updated `fetchUserModeFromDb(uid)` to sync local storage when Firebase RTDB returns `"DRIVER"` or `"PASSENGER"`, and retain the local persisted mode on network cancellation or offline state instead of forcing a reset to `PASSENGER`.
   - Updated `init` auth flow, `signUp`, `signOut`, and `continueAsGuest` to handle mode persistence cleanly.
4. **Build Verification**:
   - Verified compilation with `compile_applet` (0 build errors).

---

## 19. RESPONSIVE ACTIVE RIDE UI REFACTORING (COMPLETED)

### 📋 Summary of Refactoring
1. **Driver Mode Active Ride Controls Sheet (`DriverModeView.kt`)**:
   - Replaced rigid hardcoded action button heights with flexible `heightIn(min = 36.dp)` constraints and `weight(1f)` modifiers with `Arrangement.spacedBy(4.dp)`.
   - Set `maxLines = 1` and `softWrap = false` on action button labels (`Navigate`, `Call`, `Chat`, `Share`) to guarantee text readability and prevent clipping on compact 320dp–360dp devices and high system font scaling settings.
   - Refactored dynamic bottom action step buttons (`ARRIVED AT PICKUP`, `PIN`, `START RIDE`, `+ Toll/Extra`, `COMPLETE RIDE`) to use `heightIn(min = 52.dp, max = 58.dp)` and flexible `weight` constraints (`weight(0.30f)`, `weight(0.70f)`, `weight(0.34f)`, `weight(0.66f)`) with `Arrangement.spacedBy(8.dp)`.
   - Maintained test tags (`driver_arrived_button`, `driver_start_trip_button`, `driver_complete_trip_button`) and theme integration.

2. **Passenger Mode Active Ride Sheet (`HomeScreen.kt`)**:
   - Refactored 4-digit PIN verification code digit boxes to use flexible adaptive modifiers: `weight(1f, fill = false).widthIn(min = 36.dp, max = 48.dp).heightIn(min = 44.dp, max = 50.dp)`.
   - Updated Call Captain (`call_captain_btn`), Chat Captain (`chat_captain_btn`), Cancel Ride (`cancel_captain_ride_btn`), and Share Ride buttons to use `heightIn(min = 44.dp, max = 48.dp)` for flexible vertical arrangement.

3. **Verification**:
   - Compiled successfully via `compile_applet` with 0 errors.

---

## 20. CITY-TO-CITY PLANNED DEPARTURES UI & FIREBASE REAL DATA INTEGRATION (COMPLETED)

### 📋 Summary of Implementation
1. **Planned Departures UI (`CityToCityPassengerDeparturesContent.kt`)**:
   - Built a responsive City-to-City Scheduled Departures view strictly matching the design: top branding bar, search card with city swap button (`FROM` / `TO`), quick-city chip scroll, date/time window selectors, corridor filter chips (`All Rides`, `Via M-2 Motorway`, `Full Car`), captain cards with badges, departure/arrival timeline, toll status indicator, luggage/feature pills, seat availability visualizer, pricing / buyout badges, and "Select & Send Offer" action button.
   - Theme integration dynamically adapts all colors and surface containers to `MaterialTheme.colorScheme` / app dark and light theme settings.
   - Designed for universal responsiveness across budget compact devices (320dp–360dp) to large screens using flexible constraints and scrollable containers.

2. **Firebase Real Data Integration (`FirebaseRepository.kt`)**:
   - Removed hardcoded mock departure initializers from `localPlannedDepartures`, `localDepartureBookings`, and `localDepartureOffers`.
   - Connected `observeAllPlannedDepartures()` to read directly from the `planned_city_rides` node in Firebase Realtime Database and Firestore, providing live real-time updates when captains publish planned trips.
