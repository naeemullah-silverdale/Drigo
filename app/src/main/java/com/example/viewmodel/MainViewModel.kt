...
class MainViewModel(
    ...
    private fun getPersistedUserMode(): UserMode {
        return try {
            val prefs = com.example.DrigoApplication.instance.getSharedPreferences("drigo_driver_prefs", Context.MODE_PRIVATE)
            val modeStr = prefs.getString("user_mode", "PASSENGER")
            if (modeStr == "DRIVER") UserMode.DRIVER else UserMode.PASSENGER
        } catch (_: Exception) {
            UserMode.PASSENGER
        }
    }

    private fun persistUserMode(mode: UserMode) {
        try {
            val prefs = com.example.DrigoApplication.instance.getSharedPreferences("drigo_driver_prefs", Context.MODE_PRIVATE)
    ...
    // Default mode is persisted user mode or PASSENGER for new users
    private val _userMode = MutableStateFlow(getPersistedUserMode())
        ...
        viewModelScope.launch {
            authRepo.authStateFlow().collect { user ->
                ...
                if (user != null) {
                    ...
                    persistUserMode(UserMode.PASSENGER)
        ...
        userRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (modeStr == "DRIVER") {
                    ...
                    persistUserMode(UserMode.DRIVER)
            ...
            override fun onCancelled(error: DatabaseError) {
                // Retain locally persisted user mode on network error or offline state
            }
        })
    }
